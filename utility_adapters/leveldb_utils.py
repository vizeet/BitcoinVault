...
def getHashFromScript(script: bytes, script_type_s: str):
        if script_type_s == 'P2PKH':
                script_type = 0
                hash_size = int(binascii.hexlify(jsonobj['script'][2:3]), 16)
                hash_b = script[3: 3 + hash_size]
        elif script_type_s == 'P2SH' or script_type_s == 'P2SH-P2WPKH' or script_type_s == 'P2SH-P2WSH':
                script_type = 1
                hash_size = int(binascii.hexlify(jsonobj['script'][1:2]), 16)
                hash_b = script[2: 2 + hash_size]
        elif script_type_s == 'P2WPKH':
                script_type = 28
                size_hash = int(binascii.hexlify(jsonobj['script'][1:2]), 16)
                hash_b = script[2: 2 + size_hash]
        elif script_type_s == 'P2WSH':
                script_type = 40
                size_hash = int(binascii.hexlify(jsonobj['script'][1:2]), 16)
                hash_b = script[2: 2 + size_hash]
        return hash_b

def getBlockDepthFromHeight(height: int):
...
class LevelDBAdapter:
        ...
        def getRequiredTxnsForAmount(self, addresses: list, amount: float):
                ...
                required_hash_b_list = [pubkey_address.address2hash(address) for address in addresses]
                ret_dict = []
                while remaining_amount > 0:
                        ...
                        if prefix == b'C':
                                ...
                                if (hash_b in required_hash_b_list and jsonobj['is_coinbase'] == False) or (hash_b in required_hash_b_list and jsonobj['is_coinbase'] == True and block_depth >= 100):
                                        address = pubkey_address.pkh2address(hash_b, self.nettype)
                                        value = jsonobj['amount']
                                        print('txn_id = %s, out_index = %s, height = %d, hash = %s, address = %s, value = %d' % (txn_hash_little_endian, out_index, jsonobj['height'], bytes.decode(binascii.hexlify(hash_b)), address, value))
                                        txn_info = {}
